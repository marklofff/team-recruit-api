defmodule TeamRecruitWeb.TeamController do
  ...
  alias TeamRecruit.TeamManager.{Team, JoinRequest}
  ...
  def create(%{assigns: %{user: user}} = conn,
    %{"team" => team_params}) do
  ...
  def add_new_game(%{assigns: %{user: user}} = conn,
  ...
  def get_user_teams(%{assigns: %{user: user}} = conn, _params) do
