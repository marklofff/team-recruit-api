defmodule Database.Notifications do
  @moduledoc """
  ...
  def check_user(%Invitation{} = invitation, %Database.Accounts.User{} = user) do
  ...
  def create_invitation(%{"user" => user} = attrs \\ %{}) do
    team = Database.TeamManager.get_team!(attrs["team_id"])
    IO.inspect(user.id)
